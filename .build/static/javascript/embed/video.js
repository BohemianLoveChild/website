 ...
 * @private
 */
vjs.IS_IE = (/msie/i).test(vjs.USER_AGENT) || (/Trident/i).test(vjs.USER_AGENT);
...
vjs.Player.prototype.onEnded = function () {
  if (vjs.IS_IE) {
    this.player().trigger({ 'type': 'pause', 'bubbles': true });
  }

  if (vjs.IS_IPAD && parseFloat(vjs.IOS_VERSION) < 8) {
    VBX.Layout.reconfigVideoPlayer(this.duration(), 0);
  }
...
// http://dev.w3.org/html5/spec/video.html#dom-media-played
// Played returns a TimeRange object.
/**
 * Get a TimeRange object with the times of the video that have been played
 * @return {Object} A TimeRange object (following HTML spec)
 */
vjs.Player.prototype.played = function () {
  var played = this.techGet('played');
  return played || vjs.createTimeRange(0, 0);
};
...
vjs.Player.prototype.lastVolume = 1;
...
vjs.Player.prototype.volume = function(percentAsDecimal){
  ...
  var returnValue = (isNaN(vol)) ? 1 : vol;
  
  this.lastVolume = returnValue;
...
vjs.Player.prototype.isTechFullscreen = function () {
    return this.techGet('isFullscreen') || false;
};
 ...
 * @return {vjs.Player} self
 */
vjs.Player.prototype.exitFullscreen = function(opt_callback){
  var fsApi = vjs.browser.fullscreenAPI;
  ...
  if (fsApi) {
    ...
    this.techCall('exitFullScreen', opt_callback);
  } else {
   this.exitFullWindow();
   this.trigger('fullscreenchange');
  }
...
vjs.MuteToggle.prototype.lastVolume = 1;

vjs.MuteToggle.prototype.onClick = function () {
    var muted = this.player_.muted();
    var currentVolume = this.player_.volume();
    if (muted || currentVolume == 0)
    {
        this.player_.muted(false);
        if (muted)
        {
            this.player_.volume(Math.max(this.lastVolume, 0.1));
        }
        else
        {
            this.player_.volume(0.1);
        }
    }
    else
    {
        this.lastVolume = this.player_.volume();
...
vjs.PosterImage.prototype.src = function (url) {
  ...
  if (_backgroundSizeSupported) {
    ...
    // if there is a custom image, make the background color black for transparent custom thumbnails
    if (url != '/videojs/blank_poster.png' && url != ' ' && url.indexOf('img.youtube.com/vi/') == -1 && url.indexOf('i.ytimg.com') == -1) {
...
vjs.MediaTechController.prototype.isFullscreen = function () {
    var doc = document;
    ...
    return !!(doc.webkitIsFullScreen || doc.mozFullScreen ||
        ...
        (this.player_ != null && this.player_.tag != null && this.player_.tag.webkitDisplayingFullscreen));
};
...
vjs.Html5.prototype.play = function () {
  ...
  // IE does not dispatch "play" event if video is replayed after it has ended.
  // So we artifically dispatch the event.
  if (vjs.IS_IE) {
    this.player().trigger({ 'type': 'play', 'bubbles': true });
  }
};
vjs.Html5.prototype.pause = function(){ this.el_.pause(); };
vjs.Html5.prototype.paused = function () { return this.el_.paused || this.el_.ended; };
...
vjs.Flash['onReady'] = function (currSwf) {
  //console.log('vjs.Flash.onReady ' + currSwf);
  var el, player;
...
vjs.Flash['finish'] = function (currSwf) {
  //console.log('vjs.Flash.finish ' + currSwf);
